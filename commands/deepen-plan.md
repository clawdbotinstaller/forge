...
This command takes an existing plan (from `/forge:plan`) and enhances each section with parallel research agents. Each major element gets its own dedicated research sub-agent to find:
...
# 3. FORGE plugin skills
ls ~/.claude/plugins/cache/*/forge/*/skills/
...
**Important:** Check EVERY source. Don't assume FORGE is the only plugin. Use skills from ANY installed plugin that's relevant.
...
<thinking>
Check for documented learnings from /forge:learn. These are solved problems stored as markdown files. Spawn a sub-agent for each learning to check if it's relevant.
</thinking>

**LEARNINGS LOCATION - Check these exact folders:**

```
docs/solutions/           <-- PRIMARY: Project-level learnings (created by /forge:learn)
├── performance-issues/
│   └── *.md
├── debugging-patterns/
│   └── *.md
...
# 3. FORGE plugin agents (all subdirectories)
find ~/.claude/plugins/cache/*/forge/*/agents -name "*.md" 2>/dev/null
...
- FORGE plugin (but SKIP workflow/ agents - only use review/, research/, design/, docs/)
- ALL other installed plugins
- Any local plugins

**For FORGE plugin specifically:**
...
Research agents should also be run for relevant plan sections.
...
2. **Learnings/Solutions sub-agents** - Relevant documented learnings from /forge:learn
...
2. **Run `/forge:review`** - Get feedback from reviewers on enhanced plan
3. **Start `/forge:build`** - Begin implementing this enhanced plan
4. **Deepen further** - Run another round of research on specific sections
5. **Revert** - Restore original plan (if backup exists)

Based on selection:
- **View diff** → Run `git diff [plan_path]` or show before/after
- **`/forge:review`** → Call the /forge:review command with the plan file path
- **`/forge:build`** → Call the /forge:build command with the plan file path
- **Deepen further** → Ask which sections need more research, then re-run those agents
- **Revert** → Restore from git or backup

## Example Enhancement

**Before (from /forge:plan):**
```markdown
## Technical Approach
