...
Then restart Claude Code and run `/forge:test-xcode` again.
```
...
/forge:test-xcode MyApp-Debug

# Test after making changes
/forge:test-xcode current
```

## Integration with /forge:review

When reviewing PRs that touch iOS code, the `/forge:review` command can spawn this as a subagent:

```
Task general-purpose("Run /forge:test-xcode for scheme [name]. Build, install on simulator, test key screens, check for crashes.")
```
